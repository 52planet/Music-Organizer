        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    {
                        if (role != "Executive" && role != "Coordinator" && role != "Clerk")
                        {
                            await UserManager.AddClaimAsync(user.Id, new Claim(ClaimTypes.Role, "Staff"));
                        }
                        else
                        {
                            if (role == "Executive")
                            {
                                await UserManager.AddClaimAsync(user.Id, new Claim(ClaimTypes.Role, "Executive"));
                                await UserManager.AddClaimAsync(user.Id, new Claim(ClaimTypes.Role, "Coordinator"));
                                await UserManager.AddClaimAsync(user.Id, new Claim(ClaimTypes.Role, "Staff"));
                            }
                            else if (role != "Coordinator")
                            {
                                await UserManager.AddClaimAsync(user.Id, new Claim(ClaimTypes.Role, "Coordinator"));
                                await UserManager.AddClaimAsync(user.Id, new Claim(ClaimTypes.Role, "Clerk"));
                                await UserManager.AddClaimAsync(user.Id, new Claim(ClaimTypes.Role, "Staff"));
                            }
                            else
                            {
